...
const VALID_SECTIONS = ["about", "projects", "experiences", "interests"];
...
export default function Terminal({
      ...
      switch (command) {
        case "help": {
          push(
            ...
            mkLine("output", "  about, projects, experiences, interests"),
            mkLine("output", ""),
        ...
        case "whoami": {
          push(promptLine, mkLine("output", "alex"), mkLine("output", ""));
          break;
        }
        case "pwd": {
          ...
          if (isLong) {
            ...
            if (isHome) {
              push(
                ...
                mkLine(
                  "output",
                  `${C}[1;34mabout${C}[0m   ${C}[1;34mprojects${C}[0m   ${C}[1;34mexperiences${C}[0m   ${C}[1;34minterests${C}[0m`,
                ),
                mkLine("output", ""),
              );
            } else {
        ...
        case "cd": {
          ...
          if (!target || target === "~" || target === HOME) {
            ...
            if (VALID_SECTIONS.includes(sectionName)) {
              const newDir = `${HOME}/${sectionName}`;
              setCwd(newDir);
              setTimeout(() => onOpenWindow(sectionName), 300);
              push(promptLine, mkLine("output", ""));
            } else {
              push(
                promptLine,
        ...
        case "sudo": {
          push(promptLine, mkLine("output", "[sudo] password for alex: "));
          setTimeout(() => {
            setLines((prev) => [
              ...prev,
              mkLine(
        ...
        case "neofetch": {
          push(
            ...
            mkLine(
              "output",
              `       .-.         ${C}[1;33malex${C}[0m@${C}[1;33mportfolio${C}[0m`,
            ),
            mkLine("output", "      (o o)        ─────────────────────────"),
            mkLine(
              "output",
  ...
  const renderLine = (line: TerminalLine) => (
    ...
    >
      {line.type === "prompt" ? (
        <div className="flex">
          <span style={{ color: "#50fa7b" }}>alex</span>
